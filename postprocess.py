...
class DBPostProcess(object):
    ...
    def filter_tag_det_res(self, dt_boxes, img_height, img_width):
        dt_boxes_new = []
        for box in dt_boxes:
        ...
        for batch_idx in range(batch_size):
            ...
            for idx in range(len(text_index[batch_idx])):
                ...
                if text_prob is not None:
                    ...
                    conf_list.append(1)
            text = ''.join(char_list)
            result_list.append((text, 0 if len(conf_list) == 0 else np.mean(conf_list)))
        return result_list
...
class OCRPostProcess(object) :
    ...
    def __call__(self, ocr_res, indices, dt_boxes) :
        local_rec_res = [['', 0.0]] * len(indices)
        offset = 0
        for it in ocr_res:
            res_batch = self.decoder(it)
            for res in res_batch: 
                local_rec_res[indices[offset]] = res
                offset+=1
        rec_res=[]
        for i, it in enumerate(local_rec_res):
            if it[1] > self.threshold :
                rec_res.append([it, dt_boxes[i]])
        return rec_res

class CLSPostProcess(object) :
    def __init__(self, label_list, threshold = 0.5) :
        self.threshold = threshold
    ...
    def __call__(self, cls_res, indices, dt_boxes) :
        cls_result = self.decode(cls_res)
        local_rec_res = [['', 0.0]] * len(indices)
        offset = 0
